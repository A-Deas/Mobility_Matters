...
# Agent selection
# ---------------------------
def load_outlier_agent(top_k=5):
    metrics = pd.read_parquet(DIFF_METRICS_PATH)
    out = metrics.sort_values("agent_mape", ascending=False).head(top_k)

    bad_pid   = out.iloc[0]["p_id"]
    bad_place = out.iloc[0]["place"]
...
def main():
    agent = load_outlier_agent(top_k=5)
    plot_utah_weekly_heatmap_with_home_work_halos(
        agent,
        month=7, day_min=24, day_max=30,
        save_path="Plots/Heat Maps/max_outlier_map.png"
    )
